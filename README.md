...
A subsequent leak-free offline research sweep adds outcome-free session position, repeat-fatigue, and time-gap rerankers, then combines candidates by within-user ordinal rank. An ordered categorical full-history candidate adds a small independent correction to that ensemble. The current reproduced validation result is **`0.610110` primary** (`0.678917` GAUC, `0.541303` nDCG@5), a gain of `+0.008640` over the baseline. This is recorded under `results/verified-ordered-consensus`; it is a research ensemble rather than the lightweight checkpoint restored by the autonomous campaign engine. The requested `0.620000` target has not yet been reached, and the remaining gap is `0.009890`.

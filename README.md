...
A subsequent leak-free offline research sweep adds outcome-free session position, repeat-fatigue, and time-gap rerankers, then combines candidates by within-user ordinal rank. An ordered categorical full-history candidate adds a small independent correction. Continuous watch-ratio supervision and a lightly anchored same-session margin add complementary GAUC and top-5 corrections. The current reproduced validation result is **`0.610974` primary** (`0.680242` GAUC, `0.541707` nDCG@5), a gain of `+0.009505` over the baseline. This is recorded under `results/verified-regularized-consensus`; it is a research ensemble rather than the lightweight checkpoint restored by the autonomous campaign engine. The requested `0.620000` target has not yet been reached, and the remaining gap is `0.009026`.
